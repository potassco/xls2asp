...
list_of_types = ["auto_detect", "skip", "int", "constant", "time", "date", "datetime", "string"]
...
class TableNameError(ValueError):
    def __init__(self, table):
        ValueError.__init__(self, "Name of a tables must respect the syntax of gringo constants", table)
...
class Instance:
                ...
                for col in range(1, len(self.data[table]["rows"][r])): 
                    if not self.is_skip(table,col):
                        if not sparse or self.data[table]["rows"][r][col] != None: 
                            pred = table+'('+str(self.data[table]["rows"][1][col])+','
                            pred += str(y)+','
                            pred += str(self.data[table]["rows"][r][col])+').\n'
                            file.write(pred)
        file.write('\n')
        file.write('\n')
    ...
    def test_auto_detect(self, table, row, col, value, default=None):
        ...
        if Conversion.is_asp_constant(value):
            ...
            raise SheetRowColumnWrongTypeValueError(table, row, col, "A value is expected", value)
        ...
        for i in range(len(self.template[table]["types"])):
            ...
            if type == "skip":
                ...
                for row in self.data[table]["rows"]:
                    ...
                    self.data[table]["rows"][row][col] = test(table, row, col, value, default)
            col += 1
    ...
    def correct_matrix_xy_style(self, table, sparse=False):
        ...
        for r in self.data[table]["rows"]: 
            if r != 1:
                self.data[table]["rows"][r][0] = test(table, r, 0, self.data[table]["rows"][r][0], default_y)
        ...
        for r in self.data[table]["rows"]: 
            if r != 1:
                for col in range(1, len(self.data[table]["rows"][r])): 
                    if not self.is_skip(table,col):
                        if not sparse or self.data[table]["rows"][r][col] != None: 
                            self.data[table]["rows"][r][col] = test(table, r, col, self.data[table]["rows"][r][col], default_v)
    ...
    def ignore_empty_row(self, table):
        ...
        for row in list_empty:
            ...
            sys.stderr.write("WARNING: Row "+str(row)+" in sheet \""+table+"\"is empty, ignoring it\n")
    ...
    def locate_empty_column(self, table):
        ...
        for col in self.data[table]["skip"]: 
            sys.stderr.write("WARNING: Column "+Conversion.col2letter(col+1)+" in sheet \""+table+"\" is empty, ignoring it\n")
...
class XlsReader:
    ...
    def parse_table(self, sheet, style):
        table = sheet.title
        sys.stderr.write("Parsing Sheet \""+table+"\" with style \""+style+"\"\n")
...
def main():
    if sys.version_info<(3,5):          # temporal solution, to be removed eventually
